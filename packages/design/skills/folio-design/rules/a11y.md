...
And the figures in `@misoto22/folio/diagrams` check theirs:
...
Anything animated is behind `motion-safe` and carries `data-folio-animated`. Do
...
remember the attribute. `data-folio-animated` goes further, removing the
...
```tsx
// Correct — the package's keyframes are all prefixed `folio-`
<div data-folio-animated className="motion-safe:animate-[folio-fade-in_200ms_var(--ease-out-expo)]">
```
...
```bash
npx @misoto22/folio docs Combobox
```
