...
export const FEEDBACK_ZH: Record<string, ComponentCopyZh> = {
  ...
  skeleton: {
    ...
    practices: [
      ...
      ['ae7ab93b', '不要给零件加 animate-pulse。外框已经在动 opacity，子元素上再来一层会和它相乘，于是这个零件的节奏和它所在的形状对不上。理由不在减少动效那一头：keyframes.css 里那条规则是加在 animation-duration 和 transition-duration 上的通用下限，手写的 Tailwind 动画和别的一样会被压掉。data-folio-animated 是组件在声明自己的动效只是装饰，不是把动效取消掉的那个机制。'],
