...
export const NAVIGATION_ZH: Record<string, ComponentCopyZh> = {
  tabs: {
    ...
    anatomy: [
      ...
      { hash: '812964a6', element: '面板', description: 'TabsContent，靠 value 相等和它的标签配对。只有在它是选中项时才挂载，并且标了 data-folio-animated，所以对要求少一点动效的读者，它的入场是被直接去掉的。' },
    ],
    practices: [
  ...
  accordion: {
    ...
    anatomy: [
      ...
      { hash: 'c53aa79f', element: '面板', description: 'Radix 的 Content——一个由它的触发器命名的 role="region"，只在展开时挂载，overflow-hidden 好让量出来的高度能做动画，里面 pb-4 pe-8，这样文字停在记号那一列之前。它带着 data-folio-animated，所以对要求少一点动效的读者，展开和收起是被整个去掉的。' },
    ],
    practices: [
  ...
  collapsible: {
    ...
    anatomy: [
      ...
      { hash: '6a399fee', element: '面板', description: 'Radix 的 Content，收起时卸载，按量出来的 --radix-collapsible-content-height 做动画，所以一组很长的和一组很短的用时一样。它是一个光秃秃的 div：没有 region role，也没有自己的名字，这一点同样和手风琴不同。它和散着用的 CollapsibleContent 都带着 data-folio-animated，所以在 prefers-reduced-motion 下两者是一致的，而不是只有其中一个遵守。' },
    ],
    practices: [
